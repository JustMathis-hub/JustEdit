...
export function FreePackMediaGallery({ videoUrl, videoThumbnail, images = [], title }: FreePackMediaGalleryProps) {
  ...
  const slides: Slide[] = [
    ...
    // Fill remaining slots with placeholders to reach 4 slides
    ...Array.from(
      { length: Math.max(0, 4 - 1 - images.length) },
      (_, i) => ({
        type: 'placeholder' as const,
        label: `Background ${String(images.length + i + 1).padStart(2, '0')}`,
        gradient: SLIDE_GRADIENTS[(images.length + i + 1) % SLIDE_GRADIENTS.length],
      })
    ),
  ];
