...
/* Atmospheric dark gradients for placeholder slides */
const SLIDE_GRADIENTS = [
  ...
  'linear-gradient(135deg, #0a0a0a 0%, #111111 50%, #1e1e1e 100%)',
];

export function FreePackMediaGallery({
  videoUrl,
  title,
  slideCount = 5,
}: FreePackMediaGalleryProps) {
  const [current, setCurrent] = useState(0);
  const [isAnimating, setIsAnimating] = useState(false);
  const videoRef = useRef<HTMLVideoElement>(null);

  /* Build slides: video first, then placeholders */
  const slides: Slide[] = [
    {
    ...
    ...Array.from({ length: slideCount - 1 }, (_, i) => ({
      ...
      label: `Background ${String(i + 1).padStart(2, '0')}`,
      gradient: SLIDE_GRADIENTS[(i + 1) % SLIDE_GRADIENTS.length],
    })),
  ];
      ...
      >
        ...
        {slides.map((slide, i) => (
          <div
            ...
            style={{
              opacity: isAnimating ? (current === i ? 0 : 0) : (current === i ? 1 : 0),
        ...
        <button
          ...
          onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(0,0,0,0.7)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.2)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(0,0,0,0.45)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; }}
        >
          ...
          >
            <div className="absolute inset-0 flex items-center justify-center">
              {slide.type === 'video' ? (
                ...
                <Film size={9} className="text-white/30" />
              )}
            </div>
          </button>
        ))}
